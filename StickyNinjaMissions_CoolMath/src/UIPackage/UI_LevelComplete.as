...
//			GameVars.useFeature4 = UIX.GetTickButtonState(pageInst.Child("feature4"));
		...
		public override function InitScreen()
		{
			...
			uix_pageName = "page_levelcomplete";
...
//			UIX.AddAnimatedButton(pageInst.Child("buttonAwards"), UIX_buttonAchievementsPressed);	
			...
			textList[0].SetText("MISSION COMPLETE!");
			
				
			var ml:MissionLevel = Missions.GetCurrentMissionLevel();
			var s:String;

			var awardText:Array = new Array("Bronze","Silver", "Gold");
			
			var levelAward:int = ml.result;
			
			textList[1].SetText("Level Jumps: " + GameVars.numLevelFlicks);
			textList[2].SetText("Best Level Jumps: " + ml.bestJumps);
			
			textList[3].SetText("Jump Award: " + awardText[levelAward]);
			textList[4].SetText("(Gold="+ml.medalJumps[0]+", Silver="+ml.medalJumps[1]+" or fewer)");
			
			
			textList[5].SetText("Level Cash Earned: $"+GameVars.levelScore);
			textList[6].SetText("");
			textList[7].SetText("");
			...
			container.gameObj.xpos = container.x;
		...
		{				
			if ( Missions.currentMissionIndex == 7 && Missions.currentMissionLevelIndex == 4)
			{
			
			trace("missiona " + Missions.currentMissionIndex);
			trace("missionb " + Missions.currentMissionLevelIndex);
				UI_MovieScreen.pageToShow = 1;
